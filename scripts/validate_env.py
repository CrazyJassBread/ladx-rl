...
from zelda_env.memory import DEFAULT_ROM_PATH
...
def main() -> int:
    ...
    digest = hashlib.md5(DEFAULT_ROM_PATH.read_bytes()).hexdigest()
    ...
    try:
        ...
        for _ in range(args.steps):
            ...
            if terminated or truncated:
                observation, info = env.reset(seed=0)
        assert {"room", "player", "inventory", "event_flags", "entities"} <= info["game_state"].keys()
    finally:
        env.close()
    print(f"OK: checksum, Gym API, game state, {args.steps}-step PyBoy smoke test")
    return 0
